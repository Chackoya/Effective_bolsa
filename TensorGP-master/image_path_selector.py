...
#import scipy.misc #import imsave
...
def load_image(path, mode='RGBA', blackwhite=False):
    ...
    rgb = scipy.ndimage.imread(path, mode=mode).astype(float)[:, :, :3]
...
def image_euclidean_distance(img_a, img_b, scale_factor=255):
    def reshape(arr):
        width = len(arr)
        height = len(arr[0])
...
def ssim(img_a, img_b, blackwhite=False):
    if not blackwhite:
        ...
        return compute_ssim(img_a, img_b)
...
#     #print (glob.glob('*'))
#     imgs_p = glob.glob('phenotypes/*.png')
#     print len(imgs_p)
#
#
#     first = True
#     thresh = 0.50
#     output = 'selection-00050-rmse/'
#     try:
#         os.makedirs(output)
#     except Exception:
#         pass
